...
void extract_data(char *, int *, float *, float *);
void json_format(char *, int, float, float);
/* USER CODE BEGIN PFP */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	 ...
	 // HAL_Delay(1000);
	  ...
	  if(ptr!=NULL)
	  {
		  HAL_UART_Transmit(&huart2, rx_data, length, 1000);
		  HAL_Delay(1000);
...
void json_format(char *json_str, int time, float latitude, float longitude)
{
 ...
 /*
void HAL_UART_RxCpltback(UART_HandleTypeDef *huart2)
{
	HAL_UART_Transmit(&huart2, rx_data, 10, 1000);
	HAL_Delay(100);
}*/
/* USER CODE END 4 */
